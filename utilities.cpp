/*! \file utilities.cpp
    \brief Implementation file for model utility functions.
  ...
  util_trim (char s[])       /* IN/OUT */
{
  ...
  if (s != NULL)
  {
    ...
    s[n + 1] = '\0';
    for (n = 0; static_cast<unsigned int>(n) < strlen (s); n++)
    {
      if ((s[n] != ' ') && (s[n] != '\t' && s[n] != '\n'))
      {
        temp = &s[n];
    ...
    if (temp != s)
    {
      for (n = 0; static_cast<unsigned int>(n) <= strlen (temp); n++)
      {
        s[n] = temp[n];
      }
    }
                     ...
                     MathLib::RandomLEcuyerBD & randomgen)
{
   ...
   */
  random_int = (int)(randomgen.getRandom()*8);
                        ...
                        PIXEL value)        /* IN     */
{
  ...
  case LT:
    count =
      ((grid[igridnumcols*(i - 1) + j - 1] < value) ? 1 : 0) +
      ((grid[igridnumcols*(i - 1) + j] < value) ? 1 : 0) +
      ((grid[igridnumcols*(i - 1) + j + 1] < value) ? 1 : 0) +
      ((grid[igridnumcols*i + j - 1] < value) ? 1 : 0) +
      ((grid[igridnumcols*i + j + 1] < value) ? 1 : 0) +
      ((grid[igridnumcols*(i + 1) + j - 1] < value) ? 1 : 0) +
      ((grid[igridnumcols*(i + 1) + j] < value) ? 1 : 0) +
      ((grid[igridnumcols*(i + 1) + j + 1] < value) ? 1 : 0);
...
**
*/
void util_get_next_neighbor (const IGrid & ingrids, /* IN     */
                             int i_in,              /* IN     */
                             int j_in,              /* IN     */
                             int *i_out,            /* OUT    */
                             int *j_out,            /* OUT    */
...
*/
void
  util_map_gridpts_2_index (GRID_P in,         /* IN     */
                            GRID_P out,        /* OUT    */
                            int *lower_bound,  /* IN     */ 
                            int *upper_bound,  /* IN     */
                            int *index,        /* IN     */
...
*/
void
  util_AllCAPS (char *str_ptr)  /* IN/OUT  */
{
  unsigned int i;
...
*/
void
  util_WriteZProbGrid (Proc & runinfo,             /* IN     */
		       Scenario & scene,           /* IN     */
                       IGrid & ingrids,            /* IN     */
		       ...
		       char name[])                /* IN     */
{
  ...
  upper_bounds[0] = SEED_COLOR_INDEX + scene.GetProbabilityColorCount ();
  indices[0] = SEED_COLOR_INDEX + scene.GetProbabilityColorCount () + 1;
  ...
  if (runinfo.GetProcessingType () == PREDICTING)
  {
     ...
     */
    lower_bounds[0] = scene.GetProbabilityColorLowerBound (0);
    upper_bounds[0] = scene.GetProbabilityColorUpperBound (0);
    indices[0] = 0;
    for (i = 1; i < scene.GetProbabilityColorCount (); i++)
    {
      lower_bounds[i] = scene.GetProbabilityColorLowerBound (i);
      upper_bounds[i] = scene.GetProbabilityColorUpperBound (i);
      indices[i] = i + 2;
    }
    util_map_gridpts_2_index (z_ptr,
                              ...
                              upper_bounds,
                              indices,
                              scene.GetProbabilityColorCount ());
    ...
    util_overlay_seed (ingrids, z_prob_ptr);
  ...
  imageout.WriteGRID (z_prob_ptr,
                 colortable.GetColortable (PROBABILITY_COLORTABLE),
                 scene, ingrids,
                 filename,
                 date_str,
                 DATE_COLOR_INDEX);
...
*/
void
  util_overlay_seed (IGrid & ingrids,     /* IN     */
		     GRID_P z_prob_ptr)   /* IN/OUT */
{
  ...
  urban_seed_ptr = ingrids.GetUrbanGridPtr (__FILE__, func, __LINE__, 0);
  ...
  urban_seed_ptr = ingrids.GridRelease (
